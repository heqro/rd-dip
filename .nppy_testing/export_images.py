...
def save_img(
    ...
    img = Image.fromarray(modified_data)
    if img.mode != "RGB":
        img = img.convert("RGB")
    img.save(filename)
...
dumped_original_img = nib.load(
    "/mnt/data_drive/hrodrigo/mri_rician_noise/.nppy_testing/segmented_brains/subject04_t1w.nii_orig.nii.gz"
)
save_img(
    dumped_original_img,
    "images_exported_to_png/slice_original.png",
    is_scalar_field=False,
)

# Get scalar field
scalar_field = nib.load(
    "/mnt/data_drive/hrodrigo/mri_rician_noise/.nppy_testing/segmented_brains/subject04_t1w.nii_scalar_field.nii.gz"
)
